 ...
 *
 * A token proves "the holder is User X" to a friend who scans it. The
 * token is generated by the holder's own server call (so the User id
 * comes from the authenticated session, not from client input) and is
 * verified later when the friend POSTs `/api/profiles/:id/link`.
 *
 * Structure: `<base64url-payload>.<base64url-signature>`
 *   payload = JSON { userId, exp }            (exp = unix ms)
...
/**
 * Mint a token attesting that the caller is `userId`, valid for the
 * next {@link LINK_TOKEN_TTL_MS} milliseconds.
 */
export function createLinkToken(userId: string): {
  token: string;
  expiresAt: string;
} {
  const exp = Date.now() + LINK_TOKEN_TTL_MS;
  const payload: LinkTokenPayload = { userId, exp };
